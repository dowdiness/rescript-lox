...
let getChar = ({source, current}) => {
  if current > source->String.length {
    None
  } else {
    Some(String.charAt(source, current - 1))
  }
}

let getLexeme = ({source, start, current}) => {
  source->String.substring(~start, ~end=current)
}
...
let addStringToken = scanner => {
  ...
  let closing = scanner => {
    if isAtEnd(scanner) {
      LoxError.error(String.make(scanner.line), "Unterminated string.")
      scanner
    } else {
      let trim = scanner => {
        let literal = Value.LoxString(
          String.substring(scanner.source, ~start=scanner.start + 1, ~end=scanner.current - 1),
        )
...
let addNumberToken = scanner => {
  let rec consumeNumber = scanner => {
    ...
    } else if peek(scanner) == "." && peekNext(scanner)->isDigit {
      scanner->advanceScanner->consumeNumber
    } else {
      scanner
    }
...
let addIdentifierToken = scanner => {
  ...
  let token = switch keywords->Array.find(((keyword, _)) => keyword == text) {
  | Some((_, token)) => token
  | None => Identifier
  }
...
let scanToken = scanner => {
  ...
  | Some(char) =>
    ...
    | c if isDigit(c) => addNumberToken(scanner)
    | c if isAlpha(c) => addIdentifierToken(scanner)
    | _ => {
        LoxError.error(String.make(scanner.line), "Unexpected character.")
        scanner
      }
    }
  }
...
let tokenToString = (token: token) => {
  tokenTypeToString(token.tokenType) ++ ", " ++ token.lexeme ++ ", " ++ String.make(token.literal)
}
